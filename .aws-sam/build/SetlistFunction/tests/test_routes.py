...
async def test_get_setlists_returns_tours(client):
    ...
    with patch("services.cache.SetlistCache.get", return_value=None), \
         patch("services.cache.SetlistCache.set"):
...
async def test_get_setlists_uses_cache(client):
    ...
    with patch("services.cache.SetlistCache.get", return_value=cached_data):
        response = client.get("/api/setlists/abc123")
    assert response.status_code == 200
...
def test_generate_playlist_404_unknown_tour(client):
    with patch("services.cache.SetlistCache.get", return_value={"Other Tour": []}):
        response = client.post(
            "/api/playlist/generate",
